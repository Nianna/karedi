...
class FileActionsConfiguration {
    ...
    @Bean
    KarediAction exportAsSinglePlayerAction(AppContext appContext, DisplayContext displayContext, SongSaver songSaver) {
        return new ExportTracksAction(EXPORT_AS_SINGLEPLAYER, 1, appContext, displayContext, songSaver);
    }

    @Bean
    KarediAction exportAsDuetAction(AppContext appContext, DisplayContext displayContext, SongSaver songSaver) {
